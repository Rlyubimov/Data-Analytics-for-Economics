...
FROM organizations
WHERE
    CAST(Founded AS INTEGER) <= EXTRACT(YEAR FROM CURRENT_DATE) - 20 -- This is for firms older than 20 years
    AND Number_of_Employees > 6000; -- This is for huge firms
...
FROM
    Warehouses;
	(SELECT SUM(amount) FROM Orders DIV total_order_amount;
...
SELECT
    ...
    CASE
        ...
        WHEN COUNT(O.order_id) * 1.0 / (SELECT COUNT(*) FROM Orders) > 0.25 AND COUNT(O.order_id) * 1.0 / (SELECT COUNT(*) FROM Orders) <= 0.60 THEN '26-50% Orders'
